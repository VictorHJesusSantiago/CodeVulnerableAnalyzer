...
def _findings(report: Any) -> list[dict[str, Any]]:
    if isinstance(report, dict):
        return _jsonable(list(report.get("findings", report.get("vulnerabilities", []))))
    out = []
    for result in getattr(report, "results", []):
        for v in getattr(result, "vulnerabilities", []):
            out.append(_jsonable(v.to_dict() if hasattr(v, "to_dict") else vars(v)))
    return out


def interactive_html(report: Any, title: str = "VulnScan Dashboard", language: str = "pt-BR") -> str:
    data = _findings(report)
    payload = json.dumps(data, ensure_ascii=False).replace("</", "<\\/")
    labels = {"pt-BR": ("Buscar", "Severidade", "Achados"), "en": ("Search", "Severity", "Findings")}.get(
        language, ("Buscar", "Severidade", "Achados")
    )
...
def advanced_sarif(report: Any) -> dict[str, Any]:
    ...
    for f in findings:
        ...
        rules[rid] = {
            "id": rid,
            "name": f.get("name", rid),
            "shortDescription": {"text": f.get("description", f.get("message", rid))},
        }
        loc = {
            "physicalLocation": {
                "artifactLocation": {"uri": f.get("file_path", f.get("file", ""))},
                "region": {"startLine": max(1, int(f.get("line_number", f.get("line", 1))))},
            }
        }
        result = {
            "ruleId": rid,
            "level": {"critical": "error", "high": "error", "medium": "warning", "low": "note"}.get(
                str(f.get("severity", "")).lower(), "note"
            ),
            "message": {"text": f.get("description", f.get("message", rid))},
            "locations": [loc],
        }
        if f.get("code_flow"):
            result["codeFlows"] = [
                    ...
                    "threadFlows": [
                        {
                            "locations": [
                                {
                                    "location": {
                                        "message": {"text": x.get("message", "flow")},
                                        "physicalLocation": {
                                            "artifactLocation": {"uri": x.get("file", "")},
                                            "region": {"startLine": x.get("line", 1)},
                                        },
                                    }
                                }
                                for x in f["code_flow"]
                            ]
                        }
                    ]
                }
        ...
        if f.get("fix"):
            result["fixes"] = [
                {
                    "description": {"text": "Correção sugerida"},
                    "artifactChanges": [
                        {
                            "artifactLocation": {"uri": f.get("file_path", "")},
                            "replacements": [
                                {
                                    "deletedRegion": {"startLine": f.get("line_number", 1)},
                                    "insertedContent": {"text": f["fix"]},
                                }
                            ],
                        }
                    ],
    ...
    return {
        ...
        "runs": [
            {"tool": {"driver": {"name": "CodeVulnerableAnalyzer", "rules": list(rules.values())}}, "results": results}
        ],
    }
...
def gitlab_sast(report: Any) -> dict[str, Any]:
    ...
    vulns = []
    for _i, f in enumerate(_findings(report)):
        digest = hashlib.sha256(json.dumps(f, sort_keys=True, default=str).encode()).hexdigest()
        vulns.append(
            {
                "id": digest,
                "category": "sast",
                "name": f.get("name", f.get("rule_id", "Finding")),
                "message": f.get("description", f.get("message", "")),
                "description": f.get("description", f.get("message", "")),
                "severity": levels.get(str(f.get("severity", "info")).lower(), "Unknown"),
                "confidence": "Medium",
                "scanner": {"id": "cva", "name": "CodeVulnerableAnalyzer"},
                "location": {"file": f.get("file_path", ""), "start_line": f.get("line_number", 1)},
                "identifiers": [
                    {"type": "cva_rule", "name": f.get("rule_id", "UNKNOWN"), "value": f.get("rule_id", "UNKNOWN")}
                ],
            }
        )
    return {
        ...
        "scan": {
            "analyzer": {"id": "cva", "name": "CVA", "version": "2"},
            "scanner": {"id": "cva", "name": "CVA", "version": "2"},
            "type": "sast",
...
def scan_diff(old: Any, new: Any) -> dict[str, list[dict[str, Any]]]:
    def key(f):
        return (f.get("rule_id"), f.get("file_path", f.get("file")), f.get("line_number", f.get("line")))

    a = {key(f): f for f in _findings(old)}
    b = {key(f): f for f in _findings(new)}
    return {
        "new": [b[k] for k in b.keys() - a.keys()],
        "fixed": [a[k] for k in a.keys() - b.keys()],
        "persistent": [b[k] for k in b.keys() & a.keys()],
    }
...
def heatmap(report: Any, authors: dict[str, str] | None = None) -> list[dict[str, Any]]:
    ...
    for f in _findings(report):
        ...
        grid[file][str(f.get("severity", "info")).lower()] += 1
    return [
        {
            "file": file,
            "author": (authors or {}).get(file, "unknown"),
            "risk": c["critical"] * 10 + c["high"] * 5 + c["medium"] * 2 + c["low"],
            **c,
        }
        for file, c in sorted(grid.items())
    ]
...
def export_docx(report: Any, path: str, title: str = "Relatório Técnico de Segurança") -> None:
    ...
    body = (
        ...
        + p(f"Total de achados: {len(rows)}", "Heading1")
    )
    for f in rows:
        body += (
            p(f"{f.get('rule_id', '')} — {f.get('severity', '')}", "Heading2")
            + p(f.get("description", f.get("message", "")))
            + p(f"{f.get('file_path', '')}:{f.get('line_number', '')}")
        )
...
def export_xlsx(report: Any, path: str) -> None:
    rows = [["Regra", "Severidade", "Arquivo", "Linha", "Descrição"]] + [
        [
            f.get("rule_id", ""),
            f.get("severity", ""),
            f.get("file_path", ""),
    ...
    for ri, row in enumerate(rows, 1):
        sheet += (
            f'<row r="{ri}">'
            + "".join(
                f'<c r="{chr(65 + ci)}{ri}" t="inlineStr"><is><t>{html.escape(str(v))}</t></is></c>'
...
def export_pdf(report: Any, path: str) -> None:
    lines = ["Relatorio de Seguranca", f"Total: {len(_findings(report))}"] + [
        f"{f.get('rule_id', '')} [{f.get('severity', '')}] {f.get('file_path', '')}" for f in _findings(report)
    ]
    stream = (
        "BT /F1 10 Tf 50 780 Td "
        + ". ".join(f"({x[:100].replace('(', '[').replace(')', ']')}) Tj 0 -14 Td" for x in lines[:50])
        + " ET"
    )
    objs = [
    ...
    for i, o in enumerate(objs, 1):
        offset.append(buf.tell())
        buf.write(f"{i} 0 obj\n".encode() + o + b"\nendobj\n")
    x = buf.tell()
    buf.write(f"xref\n0 {len(objs) + 1}\n0000000000 65535 f \n".encode())
    for o in offset[1:]:
        buf.write(f"{o:010} 00000 n \n".encode())
    target = Path(path)
...
def confluence_storage(report: Any) -> str:
    return "<h1>Relatório de Segurança</h1>" + "".join(
        f"<h2>{html.escape(str(f.get('rule_id', '')))}</h2><p>{html.escape(str(f.get('description', f.get('message', ''))))}</p>"
        for f in _findings(report)
    )
    ...
    return [
        {
            "fields": {
                "project": {"key": project},
                "summary": f"[{f.get('severity', '')}] {f.get('rule_id', 'Achado')}",
                "description": f.get("description", f.get("message", "")),
                "issuetype": {"name": "Bug"},
