...
PROVIDER_SIGNATURES: list[ProviderSignature] = [
    ...
    _p(
        "AWS", "Session Token", r"\bASIA[0-9A-Z]{16}\b", "https://console.aws.amazon.com/iam/home#/security_credentials"
    ),
    _p(
        "AWS",
        "MWS Auth Token",
    ...
    _p(
        "Heroku",
        "API Key",
        r'(?i)heroku[a-z0-9_\-]*[:=]\s*["\']?[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}',
    ...
    _p(
        "GitHub", "Fine-grained PAT", r"\bgithub_pat_[A-Za-z0-9_]{82}\b", "https://github.com/settings/tokens?type=beta"
    ),
    _p(
        "GitLab",
        "Personal Access Token",
    ...
    _p(
        "Travis CI",
        "API Token",
        r'(?i)travis[_-]?token\s*[:=]\s*["\']?[A-Za-z0-9]{22}',
        "https://app.travis-ci.com/account/preferences",
    ),
    _p(
        "Terraform Cloud",
        "API Token",
        r"\b[A-Za-z0-9]{14}\.atlasv1\.[A-Za-z0-9_-]{60,90}\b",
    ...
    _p(
        ...
        r"\bpk_live_[A-Za-z0-9]{24,247}\b",
    ...
    _p(
        "PayPal/Braintree",
        "Access Token",
        r"\baccess_token\$production\$[A-Za-z0-9]{16}\$[A-Za-z0-9]{32}\b",
    ...
    _p(
        ...
        "Connection String c/ senha",
        r"postgres(?:ql)?://[^:\s]+:[^@\s]+@[^\s/]+",
        "N/A — gire a senha do banco diretamente",
        confidence="MEDIUM",
    ),
    _p(
        "MySQL",
    ...
    _p(
        ...
        "URI c/ senha",
        r"redis://[^:\s]*:[^@\s]+@[^\s/]+",
        "N/A — gire a senha do Redis diretamente",
        confidence="MEDIUM",
    ),
    _p(
        "RabbitMQ",
        "URI c/ senha",
        r"amqp://[^:\s]+:[^@\s]+@[^\s/]+",
        "N/A — gire a senha do RabbitMQ diretamente",
        confidence="MEDIUM",
    ),
    _p(
        "Snowflake",
        "Connection c/ senha",
        r'(?i)snowflake[_-]?(?:password|pwd)\s*[:=]\s*["\'][^"\']{6,}',
    ...
    _p(
        "PagerDuty",
        "API Key",
        r'(?i)pagerduty[_-]?(?:api[_-]?)?key\s*[:=]\s*["\']?[A-Za-z0-9+_-]{20}',
        "https://app.pagerduty.com/api_keys",
    ),
    _p(
        "Bugsnag",
        "API Key",
        r'(?i)bugsnag[_-]?api[_-]?key\s*[:=]\s*["\']?[a-f0-9]{32}',
        "https://app.bugsnag.com/settings/",
    ),
    _p(
        "Rollbar",
        "Access Token",
        r'(?i)rollbar[_-]?(?:access[_-]?)?token\s*[:=]\s*["\']?[a-f0-9]{32}',
        "https://rollbar.com/settings/tokens/",
    ),
    _p(
        "Honeybadger",
        "API Key",
    ...
    _p(
        "Segment",
        "Write Key",
        r'(?i)segment[_-]?write[_-]?key\s*[:=]\s*["\']?[A-Za-z0-9]{32}',
        "https://app.segment.com/",
    ),
    _p(
        "Mixpanel",
        "API Secret",
        r'(?i)mixpanel[_-]?(?:api[_-]?)?secret\s*[:=]\s*["\']?[a-f0-9]{32}',
    ...
    _p(
        "Auth0",
        "Client Secret",
        r'(?i)auth0[_-]?client[_-]?secret\s*[:=]\s*["\']?[A-Za-z0-9_-]{64}',
    ...
    _p(
        "Twitter/X",
        "Bearer Token",
        r"\bAAAAAAAAAAAAAAAAAAAAAA[A-Za-z0-9%]{20,}\b",
    ...
    _p(
        ...
        "https://support.zendesk.com/hc/en-us/articles/4408886858522",
    ),
    _p(
        "Intercom",
        "Access Token",
        r'(?i)intercom[_-]?(?:access[_-]?)?token\s*[:=]\s*["\']?[A-Za-z0-9=_-]{60,}',
    ...
    _p(
        ...
        "N/A — invalide o token no serviço emissor",
        confidence="MEDIUM",
    ),
    _p(
        "SSH",
        ...
        "N/A — remova a chave de todo host autorizado (~/.ssh/authorized_keys)",
    ),
    _p(
        "PGP",
        "Private Key Block",
        r"-----BEGIN PGP PRIVATE KEY BLOCK-----",
        "N/A — revogue via seu certificado de revogação PGP",
    ),
    _p(
        "Generic",
        "API Key/Secret Atribuído",
        r'(?i)\b(?:api[_-]?key|secret[_-]?key|access[_-]?token)\s*[:=]\s*["\'][A-Za-z0-9_\-/+=]{16,}["\']',
        "N/A — revise manualmente o serviço correspondente",
        confidence="LOW",
    ),
    _p(
        "Generic",
    ...
    _p(
        "Netlify",
        "Access Token",
        r'(?i)netlify[_-]?(?:auth[_-]?|access[_-]?)?token\s*[:=]\s*["\']?[A-Za-z0-9_-]{40,64}',
    ...
    _p(
        "Fly.io",
        "API Token",
        r'(?i)fly[_-]?api[_-]?token\s*[:=]\s*["\']?[A-Za-z0-9_/=+]{40,}',
    ...
    _p(
        "Contentful",
        "Management Token",
        r'(?i)contentful[_-]?(?:management[_-]?)?token\s*[:=]\s*["\']?[A-Za-z0-9_-]{40,}',
    ...
    _p(
        "LaunchDarkly",
        "SDK Key",
        r"\bsdk-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b",
    ...
    _p(
        "Freshdesk",
        "API Key",
        r'(?i)freshdesk[_-]?api[_-]?key\s*[:=]\s*["\']?[A-Za-z0-9]{15,}',
        "https://support.freshdesk.com/",
    ),
    _p(
        "HubSpot",
        "API Key/Token",
        r'(?i)hubspot[_-]?(?:api[_-]?key|token)\s*[:=]\s*["\']?[a-f0-9-]{36}',
        "https://app.hubspot.com/private-apps/",
    ),
    _p(
        "Salesforce",
        "Session ID/Token",
        r'(?i)salesforce[_-]?(?:session[_-]?id|token)\s*[:=]\s*["\']?[A-Za-z0-9._!]{60,}',
        "https://login.salesforce.com/",
    ),
    _p(
        "Zapier",
        "Webhook Secret",
        r"https://hooks\.zapier\.com/hooks/catch/\d+/[A-Za-z0-9]+",
    ...
    _p(
        "Coinbase",
        "API Secret",
        r'(?i)coinbase[_-]?(?:api[_-]?)?secret\s*[:=]\s*["\']?[A-Za-z0-9]{64,}',
    ...
    _p(
        "Plaid", "Secret", r'(?i)plaid[_-]?secret\s*[:=]\s*["\']?[a-f0-9]{30,}', "https://dashboard.plaid.com/team/keys"
    ),
    _p(
        "Twitch",
        "Client Secret",
        r'(?i)twitch[_-]?client[_-]?secret\s*[:=]\s*["\']?[a-z0-9]{30}',
    ...
    _p(
        "Figma",
        "Personal Access Token",
        r"\bfigd_[A-Za-z0-9_-]{40,}\b",
        "https://www.figma.com/developers/api#access-tokens",
    ),
    _p(
        "CloudAMQP",
        "URI c/ senha",
        r"amqps://[^:\s]+:[^@\s]+@[^\s/]+\.cloudamqp\.com",
        "https://customer.cloudamqp.com/",
    ),
    _p(
        "Elastic Cloud",
        "API Key",
        r'(?i)elastic[_-]?(?:cloud[_-]?)?api[_-]?key\s*[:=]\s*["\']?[A-Za-z0-9+/=]{40,}',
    ...
    _p(
        "Doppler",
        "Service Token",
        r"\bdp\.st\.[A-Za-z0-9_]{40,44}\b",
        "https://dashboard.doppler.com/workplace/settings/tokens",
    ),
    _p(
        "Infura",
        "Project ID",
        r'(?i)infura[_-]?(?:project[_-]?)?id\s*[:=]\s*["\']?[a-f0-9]{32}',
        "https://app.infura.io/",
    ),
    _p(
        "Alchemy",
...
for _provider in _CONTEXTUAL_PROVIDERS:
    ...
    PROVIDER_SIGNATURES.append(
        _p(
            _provider,
            "API Key/Token contextual",
            rf'(?i)\b{_slug}[_-]?(?:api[_-]?)?(?:key|token|secret)\s*[:=]\s*["\'][A-Za-z0-9_./+=-]{{16,}}["\']',
            "N/A — abra o painel de credenciais do provedor",
            confidence="MEDIUM",
        )
    )
