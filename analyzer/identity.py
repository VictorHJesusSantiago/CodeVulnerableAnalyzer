...
class IdentityGraph:
    ...
    def add_principal(self, p: Principal) -> None:
        self.principals[p.id] = p

    def add_edge(self, e: PrivilegeEdge) -> None:
        if e.source not in self.principals or e.target not in self.principals:
            raise KeyError("Aresta referencia principal desconhecido")
        self.edges.append(e)
    ...
    def paths(self, start: str, goal: str, max_depth: int = 8) -> list[list[PrivilegeEdge]]:
        ...
        while queue:
            ...
            if len(path) >= max_depth:
                continue
            for e in self.edges:
                if e.source != node or e.target in seen:
                    continue
                next_path = path + [e]
                if e.target == goal:
                    result.append(next_path)
                else:
                    queue.append((e.target, next_path, seen | {e.target}))
        return result

    def escalation_paths(self, targets: set[str] | None = None) -> list[dict[str, Any]]:
        targets = targets or {
            p.id for p in self.principals.values() if p.attributes.get("tier0") or p.kind in {"domain", "account-root"}
        }
        out = []
    ...
    def simulate_removal(self, source: str, target: str, relation: str) -> dict[str, Any]:
        before = len(self.escalation_paths())
        kept = [e for e in self.edges if not (e.source == source and e.target == target and e.relation == relation)]
        old = self.edges
        self.edges = kept
        after = len(self.escalation_paths())
        self.edges = old
    ...
    def to_bloodhound(self) -> dict[str, Any]:
        return {
            "nodes": [
                {"id": p.id, "type": p.kind, "provider": p.provider, **p.attributes} for p in self.principals.values()
            ],
            "edges": [
                {"source": e.source, "target": e.target, "label": e.relation, "permissions": sorted(e.permissions)}
                for e in self.edges
            ],
        }
...
def detect_identity_risks(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def hit(rule, severity, principal, message):
        out.append({"rule_id": rule, "severity": severity, "principal": principal, "message": message})

    for user in snapshot.get("users", []):
        uid = user.get("id", user.get("name", "unknown"))
        if not user.get("mfa", False):
            hit("IAM-MFA-001", "high", uid, "MFA ausente")
        if user.get("preauth_disabled"):
            hit("AD-KERBEROAST-001", "high", uid, "Pré-autenticação Kerberos desabilitada")
        if user.get("admin") and user.get("synced"):
            hit("IAM-SHADOW-001", "critical", uid, "Administrador indireto/sincronizado")
    for sp in snapshot.get("service_principals", []):
        sid = sp.get("id", "unknown")
        if sp.get("secrets"):
            hit("AAD-SP-SECRET-001", "high", sid, "Service principal usa segredo estático")
        if "*" in sp.get("permissions", []):
            hit("IAM-WILDCARD-001", "critical", sid, "Permissão global")
...
def import_aws_iam(doc: dict[str, Any]) -> IdentityGraph:
    ...
    for p in doc.get("principals", []):
        g.add_principal(Principal(p["arn"], p.get("type", "principal"), "aws", p))
    for r in doc.get("roles", []):
        if r["arn"] not in g.principals:
            g.add_principal(Principal(r["arn"], "role", "aws", r))
        for src in r.get("trusted", []):
            if src not in g.principals:
                g.add_principal(Principal(src, "external", "aws"))
...
def import_directory(doc: dict[str, Any], provider: str) -> IdentityGraph:
    ...
    g = IdentityGraph()
    for item in doc.get("principals", []):
        g.add_principal(Principal(item["id"], item.get("kind", "user"), provider, item.get("attributes", {})))
    for item in doc.get("relationships", []):
        g.add_edge(
            PrivilegeEdge(
                item["source"],
                item["target"],
                item["relation"],
                set(item.get("permissions", [])),
                item.get("metadata", {}),
            )
        )
    return g
