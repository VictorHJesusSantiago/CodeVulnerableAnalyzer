...
class ScanService:
    def __init__(self, scanner: Callable[[str], Any]):
        self.scanner = scanner
        self.jobs: dict[str, dict[str, Any]] = {}

    def submit(self, path: str) -> str:
        job = str(uuid.uuid4())
        self.jobs[job] = {"id": job, "status": "queued", "path": path}
    ...
    def _run(self, job: str) -> None:
        ...
        try:
            self.jobs[job].update(status="completed", result=self.scanner(self.jobs[job]["path"]))
        except Exception as e:
            self.jobs[job].update(status="failed", error=str(e))
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/api/v1/jobs":
                return self._json(200, list(service.jobs.values()))
            if self.path.startswith("/api/v1/jobs/"):
                return self._json(200, service.jobs.get(self.path.rsplit("/", 1)[-1], {"error": "not_found"}))
        ...
        def do_POST(self):
            ...
            if self.path == "/api/v1/scans":
                return self._json(202, {"job_id": service.submit(body["path"])})
            if self.path == "/graphql":
                query = body.get("query", "")
                if "jobs" in query:
                    return self._json(200, {"data": {"jobs": list(service.jobs.values())}})
                if "scan" in query:
                    return self._json(
                        200, {"data": {"scan": {"job_id": service.submit(body.get("variables", {}).get("path", "."))}}}
                    )
