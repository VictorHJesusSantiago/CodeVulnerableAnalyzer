...
def map_finding(finding: dict[str, Any]) -> dict[str, list[str]]:
    cwe = finding.get("cwe", "")
    return {
        fw: [control for control, cwes in controls.items() if cwe in cwes]
        for fw, controls in FRAMEWORKS.items()
        if any(cwe in c for c in controls.values())
    }
...
def compliance_report(findings: Iterable[dict[str, Any]], framework: str) -> dict[str, Any]:
    ...
    for control, cwes in controls.items():
        ...
        rows.append(
            {
                "control": control,
                "status": "fail" if related else "pass",
    ...
    return {
        "framework": framework,
        "score": round(100 * passed / max(1, len(rows)), 1),
        "controls": rows,
        "gaps": [r["control"] for r in rows if r["status"] == "fail"],
    }
