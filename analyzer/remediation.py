...
class RemediationEngine:
    def __init__(self):
        self.codemods: dict[str, Codemod] = {}

    def register(self, rule_id: str, codemod: Codemod) -> None:
        self.codemods[rule_id] = codemod
    ...
    def plan(self, file_path: str, source: str, findings: Iterable[dict[str, Any]]) -> Patch:
        ...
        for f in findings:
            ...
            if transform:
                edits.extend(transform(source, f))
        self._validate_edits(edits, len(source.splitlines()))
        updated = apply_edits(source, edits)
        diff = "".join(
            difflib.unified_diff(source.splitlines(True), updated.splitlines(True), f"a/{file_path}", f"b/{file_path}")
        )
        return Patch(
            file_path,
            ...
            edits,
            diff,
            f"{len(edits)} correções determinísticas",
        )

    def apply(self, patch: Patch, root: str | Path = ".", dry_run: bool = False) -> str:
        path = (Path(root) / patch.file_path).resolve()
        base = Path(root).resolve()
        if base not in path.parents and path != base:
            raise ValueError("Patch tenta sair da raiz")
        source = path.read_text(encoding="utf-8")
        if hashlib.sha256(source.encode()).hexdigest() != patch.before_sha256:
            raise RuntimeError("Arquivo mudou desde a geração do patch")
    ...
    def _validate_edits(edits: list[TextEdit], lines: int) -> None:
        ...
        for i, e in enumerate(ordered):
            if e.start_line < 1 or e.end_line < e.start_line or e.end_line > max(1, lines):
                raise ValueError("Intervalo de edição inválido")
            if i and ordered[i - 1].end_line >= e.start_line:
...
def apply_edits(source: str, edits: Iterable[TextEdit]) -> str:
    ...
    for e in sorted(edits, key=lambda x: x.start_line, reverse=True):
        newline = (
            "\n"
            if lines[e.start_line - 1 : e.end_line]
            and lines[e.start_line - 1 : e.end_line][-1].endswith("\n")
...
def python_yaml_load_codemod(source: str, finding: dict[str, Any]) -> list[TextEdit]:
    return _line_edit(
        source, finding, lambda line: re.sub(r"\byaml\.load\s*\(([^,\n]+)\)", r"yaml.safe_load(\1)", line)
    )
...
def python_weak_hash_codemod(source: str, finding: dict[str, Any]) -> list[TextEdit]:
    ...
    return _line_edit(source, finding, lambda line: re.sub(r"hashlib\.(?:md5|sha1)\s*\(", "hashlib.sha256(", line))
...
def flask_debug_codemod(source: str, finding: dict[str, Any]) -> list[TextEdit]:
    ...
    return _line_edit(source, finding, lambda line: re.sub(r"\bdebug\s*=\s*True\b", "debug=False", line))
...
def python_insecure_random_token_codemod(source: str, finding: dict[str, Any]) -> list[TextEdit]:
    ...
    def _fix(line: str) -> str:
        ...
        if not any(
            k in lhs
            for k in ("token", "secret", "password", "senha", "csrf", "sessionid", "session_id", "apikey", "api_key")
        ):
            return line
...
def javascript_dangerously_set_innerhtml_codemod(source: str, finding: dict[str, Any]) -> list[TextEdit]:
    ...
    if n < 1 or n > len(lines):
        return []
    line = lines[n - 1]
    if "vulnscan: revisar XSS" in line:
        return []
...
class AssistedRemediator:
    ...
    def suggest(self, finding: dict[str, Any], code: str) -> dict[str, str]:
        prompt = (
            "Você é um remediador de segurança. Não altere comportamento além do necessário. "
        ...
        try:
            ...
            return {"explanation": raw, "replacement": ""}
        return {"explanation": str(data.get("explanation", "")), "replacement": str(data.get("replacement", ""))}


def lsp_code_actions(
    uri: str, source: str, findings: Iterable[dict[str, Any]], engine: RemediationEngine | None = None
) -> list[dict[str, Any]]:
    ...
    for f in findings:
        ...
        for edit in mod(source, f):
            actions.append(
                {
                    "title": f"VulnScan: corrigir {f['rule_id']}",
                    "kind": "quickfix",
                    "diagnostics": [{"code": f["rule_id"]}],
                    "edit": {
                        "changes": {
                            uri: [
                                {
                                    "range": {
                                        "start": {"line": edit.start_line - 1, "character": 0},
...
def create_remediation_pr(
    provider: PullRequestProvider, patches: Iterable[Patch], contents: dict[str, str]
) -> dict[str, Any]:
    patches = list(patches)
    changes = {p.file_path: apply_edits(contents[p.file_path], p.edits) for p in patches}
