...
def _worklist(cfg: CFG, forward: bool, gen_kill) -> dict[int, tuple[set, set]]:
...
def halstead_metrics(func: ast.AST) -> dict:
    ...
    for node in ast.walk(func):
        if isinstance(node, ast.BinOp):
            ...
            node, (ast.If, ast.For, ast.While, ast.Return, ast.Import, ast.ImportFrom, ast.With, ast.Lambda)
        ):
...
def _self_recursive_without_base_case(func: ast.FunctionDef) -> bool:
    ...
    calls_self = any(
        isinstance(n, ast.Call) and isinstance(n.func, ast.Name) and n.func.id == func.name for n in ast.walk(func)
    )
    if not calls_self:
        return False
    has_return_or_yield = any(isinstance(n, (ast.Return, ast.Yield, ast.YieldFrom)) for n in ast.walk(func))
...
def analyze_python_ast(file_path: str, content: str) -> list[Vulnerability]:
    ...
    for stmt in _find_unreachable(tree):
        results.append(
            Vulnerability(
                rule_id="AST-DEAD-001",
                name="Código Inalcançável (Dead Code)",
                ...
                line_content=_line(stmt.lineno),
    ...
    for node in ast.walk(tree):
        ...
        if cyclo > MAX_CYCLOMATIC_AST:
            results.append(
                Vulnerability(
                    rule_id="AST-CMPLX-001",
                    name="Complexidade Ciclomática Alta (via AST)",
                    ...
                    line_content=_line(node.lineno),
        ...
        if cognitive > MAX_COGNITIVE_AST:
            results.append(
                Vulnerability(
                    rule_id="AST-CMPLX-002",
                    name="Complexidade Cognitiva Alta (via AST)",
        ...
        if _self_recursive_without_base_case(node):
            results.append(
                Vulnerability(
                    rule_id="AST-REC-001",
                    name="Recursão Sem Caso-Base Aparente",
        ...
        for check_call, open_call in _toctou_pairs(node):
            results.append(
                Vulnerability(
                    rule_id="AST-TOCTOU-001",
                    name="Race Condition TOCTOU (check-then-use)",
                    ...
                    language=Language.PYTHON,
                    file_path=file_path,
                    line_number=open_call.lineno,
                    line_content=_line(open_call.lineno),
        ...
        for name, lineno in _use_after_close(node):
            results.append(
                Vulnerability(
                    rule_id="AST-UAC-001",
                    name="Uso Após close() (Use-After-Close)",
        ...
        for name, lineno in _null_deref_candidates(node):
            results.append(
                Vulnerability(
                    rule_id="AST-NULL-001",
                    name="Possível Acesso a Atributo de None",
        ...
        if cfg.nodes:
            ...
            for nid, cfgnode in cfg.nodes.items():
                ...
                for varname in dead_stores:
                    ...
                    results.append(
                        Vulnerability(
                            rule_id="AST-DATAFLOW-001",
                            name="Atribuição Nunca Utilizada (Dead Store)",
                            ...
                            language=Language.PYTHON,
                            file_path=file_path,
                            line_number=cfgnode.stmt.lineno,
            ...
            for nid, cfgnode in cfg.nodes.items():
                ...
                for varname in candidates:
                    ...
                    results.append(
                        Vulnerability(
                            rule_id="AST-SSA-001",
                            name="Variável Possivelmente Não Inicializada (via SSA)",
