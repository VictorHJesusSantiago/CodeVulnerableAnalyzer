...
def query_deps_dev(system: str, package: str, version: str) -> list[Advisory]:
    key = urllib.parse.quote(package, safe="")
    doc = _get_json(f"https://api.deps.dev/v3/systems/{system}/packages/{key}/versions/{version}")
    return [
        Advisory(a.get("advisoryKey", {}).get("id", "unknown"), package, system, "UNKNOWN", "", [], "deps.dev")
        for a in doc.get("advisoryKeys", [])
    ]
...
def query_nvd(cpe: str, api_key: str = "") -> list[Advisory]:
    ...
    doc = _get_json(
        "https://services.nvd.nist.gov/rest/json/cves/2.0?cpeName=" + urllib.parse.quote(cpe, safe=":/"), headers
    )
    out = []
        ...
        for name in ("cvssMetricV31", "cvssMetricV30", "cvssMetricV2"):
            if metrics.get(name):
                sev = (
                    metrics[name][0]
                    .get("cvssData", {})
                    .get("baseSeverity", metrics[name][0].get("baseSeverity", "UNKNOWN"))
                )
                break
...
def query_oss_index(coordinates: list[str], username: str, token: str) -> list[Advisory]:
    body = json.dumps({"coordinates": coordinates}).encode()
    ...
    for component in doc:
        for v in component.get("vulnerabilities", []):
            out.append(
                Advisory(
                    v.get("id", ""),
