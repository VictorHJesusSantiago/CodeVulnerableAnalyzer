...
def scan_mobile_archive(path: str | Path, max_entry_bytes: int = 8 * 1024 * 1024) -> dict[str, Any]:
    path = Path(path)
    kind = "apk" if path.suffix.lower() == ".apk" else "ipa" if path.suffix.lower() == ".ipa" else "archive"
    findings = []
    files = []
    certificates = []
    with zipfile.ZipFile(path) as archive:
        for info in archive.infolist():
            ...
            if info.file_size > max_entry_bytes:
                findings.append(
                    {
                        "rule_id": "MOBILE-ZIP-001",
                        "severity": "medium",
                        "file": info.filename,
                        "message": "Entrada excede limite de inspeção",
                    }
                )
                continue
            ...
            if normalized.is_absolute() or ".." in normalized.parts:
                findings.append(
                    {"rule_id": "MOBILE-ZIP-002", "severity": "critical", "file": info.filename, "message": "Zip Slip"}
                )
                continue
            ...
            if info.filename.endswith("AndroidManifest.xml") and raw.lstrip().startswith(b"<"):
                findings.extend(
                    {**x, "file": info.filename} for x in scan_android_manifest(raw.decode("utf-8", "replace"))
                )
            if info.filename.endswith("Info.plist"):
                try:
                    ...
                    if ats.get("NSAllowsArbitraryLoads"):
                        findings.append(
                            {
                                "rule_id": "MOBILE-IOS-ATS-001",
                                "severity": "high",
                                "file": info.filename,
                                "message": "ATS permite cargas arbitrárias",
                            }
                        )
                    ...
                    if schemes and not plist.get("CFBundleURLName"):
                        findings.append(
                            {
                                "rule_id": "MOBILE-IOS-LINK-001",
                                "severity": "medium",
                                "file": info.filename,
                                "message": "URL scheme deve validar origem",
                            }
                        )
            ...
            if suffix in TEXT_EXT:
                ...
                for rid, pattern, sev, msg in [
                    ...
                    (
                        "MOBILE-DEEPLINK-001",
                        r"(?:scheme|CFBundleURLSchemes).{0,80}(?:http|custom)",
                        "medium",
                        "Deep link deve validar origem e parâmetros",
                    ),
                ]:
                    if re.search(pattern, text, re.I | re.S):
                        findings.append({"rule_id": rid, "severity": sev, "file": info.filename, "message": msg})
    if kind == "apk" and not certificates:
        findings.append(
            {
                "rule_id": "MOBILE-SIGN-001",
                "severity": "high",
                "file": str(path),
                "message": "Assinatura v1 não encontrada; valide esquemas v2/v3 externamente",
            }
        )
...
def decompile_apk(
    path: str | Path, output_dir: str | Path, apktool_binary: str = "apktool", timeout: int = 120
) -> Path:
    ...
    try:
        result = subprocess.run(
            [apktool_binary, "decode", "--force", "--output", str(output), str(path)],
            capture_output=True,
            text=True,
            timeout=timeout,
            check=False,
        ...
        raise RuntimeError("apktool não encontrado") from exc
    except subprocess.TimeoutExpired as exc:
        raise RuntimeError("apktool excedeu o timeout") from exc
    if result.returncode:
        raise RuntimeError(result.stderr.strip() or "Falha no apktool")
    return output
...
def scan_decompiled_mobile(root: str | Path) -> dict[str, Any]:
    ...
    for path in root.rglob("*"):
        ...
        if path.name == "AndroidManifest.xml":
            findings.extend({**x, "file": str(path)} for x in scan_android_manifest(text))
