...
def _call_task(task: tuple[Callable[[str], Any], str]) -> tuple[str, Any]:
    ...
    try:
        ...
        return path, {"error": type(e).__name__ + ": " + str(e)}


def parallel_map(
    scanner: Callable[[str], Any], paths: Sequence[str], workers: int | None = None, timeout: float = 30
) -> dict[str, Any]:
    result = {}
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers or max(1, (os.cpu_count() or 2) - 1)) as pool:
        ...
        for future, p in futures.items():
            try:
                _, value = future.result(timeout=timeout)
                result[p] = value
            except concurrent.futures.TimeoutError:
                future.cancel()
                result[p] = {"error": "timeout"}
    return result
...
class ASTCache:
    def __init__(self, path: str):
        ...
        with sqlite3.connect(path) as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS ast_cache(hash TEXT PRIMARY KEY,dump TEXT NOT NULL,created REAL NOT NULL)"
            )
    ...
    def parse(self, source: str) -> ast.AST:
        ...
        with sqlite3.connect(self.path) as db:
            ...
            db.execute(
                ...
                (key, ast.dump(tree, include_attributes=True), time.time()),
            )
            return tree
    ...
    def contains(self, source: str) -> bool:
        key = hashlib.sha256(source.encode()).hexdigest()
        with sqlite3.connect(self.path) as db:
            return db.execute("SELECT 1 FROM ast_cache WHERE hash=?", (key,)).fetchone() is not None
    ...
    def __init__(self, rules: Sequence[tuple[str, str, int]]):
        ...
        for i, (rid, pattern, flags) in enumerate(rules):
            if flags & ~re.IGNORECASE:
                self.fallback.append((rid, re.compile(pattern, flags)))
                continue
            try:
                re.compile(pattern, flags)
                parts.append(f"(?P<R{i}>{pattern})")
            except re.error:
                self.fallback.append((rid, re.compile(r"(?!x)x")))
        if parts:
            self.combined = re.compile(
                "|".join(parts), re.IGNORECASE if any(x[2] & re.IGNORECASE for x in rules) else 0
            )
    ...
    def scan(self, text: str) -> list[dict[str, Any]]:
        ...
        if self.combined:
            for m in self.combined.finditer(text):
                ...
                out.append({"rule_id": self.rules[idx][0], "start": m.start(), "end": m.end()})
...
class ResourceLimits:
    ...
    def validate_file(self, path: str | Path) -> None:
        if Path(path).stat().st_size > self.max_bytes:
...
class DistributedCoordinator:
    def __init__(self, queue: Any, result_queue: Any):
        self.queue, self.results = queue, result_queue

    def submit(self, paths: Iterable[str]) -> list[str]:
        ids = []
        for path in paths:
            job = hashlib.sha256(f"{path}:{time.time_ns()}".encode()).hexdigest()[:16]
            self.queue.publish("scan.jobs", {"id": job, "path": path})
            ids.append(job)
        return ids


class DistributedWorker:
    def __init__(self, queue: Any, scanner: Callable[[str], Any]):
        self.queue, self.scanner = queue, scanner

    def run_once(self) -> None:
        def handle(job):
            self.queue.publish("scan.results", {"id": job["id"], "result": self.scanner(job["path"])})
