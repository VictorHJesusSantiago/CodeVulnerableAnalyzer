...
def scan_dockerfile(text: str) -> list[dict[str, Any]]:
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        line = raw.strip()
        if re.match(r"USER\s+\S+", line, re.I):
            user_seen = not bool(re.match(r"USER\s+(?:0|root)\b", line, re.I))
        if re.match(r"FROM\s+\S+:latest\b", line, re.I):
            add("DOCKER-BP-001", "medium", n, "Imagem base usa tag latest")
        if re.match(r"ADD\s+https?://", line, re.I):
            add("DOCKER-BP-002", "high", n, "ADD remoto sem verificação de integridade")
        if re.search(r"COPY\s+.*(?:\.env|id_rsa|\.pem|credentials|secret)", line, re.I):
            add("DOCKER-BP-003", "critical", n, "Possível segredo copiado para a imagem")
        if re.match(r"RUN\s+.*(?:apt-get|apk|yum).*(?:install)", line, re.I) and not re.search(
            r"(?:rm -rf /var/lib/apt/lists|--no-cache)", line
        ):
            add("DOCKER-BP-004", "low", n, "Camada mantém cache do gerenciador de pacotes")
    if not user_seen:
        add("DOCKER-BP-005", "high", len(text.splitlines()) or 1, "Imagem termina executando como root")
    return findings
...
def scan_compose(doc: str | dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for name, svc in data.get("services", {}).items():
        for rule, condition, message in [
            ...
            if condition:
                out.append({"rule_id": rule, "service": name, "severity": "high", "message": message})
    return out
            ...
            with tarfile.open(fileobj=member) as archive:
                for item in archive.getmembers():
                    if re.search(r"(?:^|/)(?:\.env|id_rsa|credentials|shadow|.*\.pem)$", item.name, re.I):
                        out.append(
                            {"rule_id": "OCI-LAYER-SECRET", "severity": "critical", "layer": index, "path": item.name}
                        )
    return out
...
def _simple_yaml(text: str) -> dict[str, Any]:
    ...
    for raw in text.splitlines():
        ...
        if raw.startswith("  ") and not raw.startswith("    ") and s.endswith(":"):
            current = s[:-1]
            result["services"][current] = {}
        elif current and ":" in s:
            k, v = s.split(":", 1)
            result["services"][current][k] = (
                v.strip().lower() == "true" if v.strip().lower() in ("true", "false") else v.strip()
            )
    return result
