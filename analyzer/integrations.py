...
def signed_webhook(url: str, event: dict[str, Any], secret: str, timeout: int = 10) -> int:
    ...
    req = urllib.request.Request(
        url, body, {"Content-Type": "application/json", "X-VulnScan-Signature": "sha256=" + sig}
    )
...
def slack_payload(summary: str, url: str = "") -> dict[str, Any]:
    return {
        ...
        "blocks": [
            ...
            {
                "type": "actions",
                "elements": [{"type": "button", "text": {"type": "plain_text", "text": "Abrir relatório"}, "url": url}],
            },
        ]
        if url
        ...
        "attachments": [
            {
                ...
                "content": {
                    ...
                    "actions": [{"type": "Action.OpenUrl", "title": "Abrir relatório", "url": url}] if url else [],
                },
            }
        ],
    }
...
def discord_payload(summary: str, url: str = "") -> dict[str, Any]:
    return {"content": summary, "embeds": [{"title": "VulnScan", "url": url, "description": summary}]}
...
class MemoryQueue:
    ...
    def consume(self, topic: str, handler: Callable[[dict[str, Any]], None]) -> None:
        while self.messages.get(topic):
            handler(self.messages[topic].pop(0))


class AdapterQueue:
    """Adaptador para produtores/consumidores Kafka ou RabbitMQ injetados."""

    def __init__(self, producer: Callable[[str, bytes], None], consumer: Callable[[str], Iterable[bytes]]):
        self.producer, self.consumer = producer, consumer

    def publish(self, topic: str, payload: dict[str, Any]) -> None:
        self.producer(topic, json.dumps(payload).encode())

    def consume(self, topic: str, handler: Callable[[dict[str, Any]], None]) -> None:
        for raw in self.consumer(topic):
...
class QualityGate:
    ...
    def evaluate(self, findings: Iterable[dict[str, Any]], new_count: int = 0, score: float = 100) -> dict[str, Any]:
        ...
        counts = {}
        for f in rows:
            key = str(f.get("severity", "info")).lower()
        ...
        if len(rows) > self.max_total:
            failures.append("total")
        if not self.allow_new and new_count:
            failures.append("new_findings")
...
def scm_check(provider: str, sha: str, gate: dict[str, Any], details_url: str = "") -> dict[str, Any]:
    ...
    if provider == "gitlab":
        return {"sha": sha, "state": state, "name": "VulnScan", "target_url": details_url}
    if provider == "bitbucket":
        return {"key": "VULNSCAN", "state": "SUCCESSFUL" if gate["passed"] else "FAILED", "url": details_url}
    ...
    return [
        {
            ...
            "body": f"**{f.get('rule_id', 'VulnScan')}**: {f.get('description', f.get('message', ''))}",
        }
        for f in findings
    ]
