...
CONTENT_SIGNATURES: list[tuple[Language, re.Pattern]] = [
    (
        Language.PYTHON,
        re.compile(
            r"^\s*(?:import\s+\w+|from\s+\w+\s+import|def\s+\w+\s*\(|class\s+\w+.*:)|^#!/usr/bin/(?:env\s+)?python"
        ),
    ),
    ...
    (
        Language.POWERSHELL,
        re.compile(r"(?:Write-Host|Get-\w+|Set-\w+|Invoke-\w+|param\s*\(|\$PSVersion)", re.IGNORECASE),
    ),
    (
        Language.SQL,
        re.compile(r"^\s*(?:SELECT|INSERT\s+INTO|CREATE\s+TABLE|UPDATE\s+\w+\s+SET|DELETE\s+FROM)", re.IGNORECASE),
    ),
    ...
    (
        Language.GRAPHQL,
        re.compile(r"^\s*(?:type|query|mutation|subscription|fragment|schema)\s+\w+\s*[\{(]", re.MULTILINE),
    ),
