...
def chacha20poly1305_decrypt(key: bytes, nonce: bytes, cipher: bytes, tag: bytes, aad: bytes = b"") -> bytes:
    ...
    if not hmac.compare_digest(expected, tag):
        raise VaultSecurityError("Tag AEAD inválida")
...
def shamir_split(secret: bytes, shares: int, threshold: int) -> list[str]:
    ...
    if not 2 <= threshold <= shares <= 255:
        raise ValueError("Use 2 <= threshold <= shares <= 255")
...
def shamir_combine(parts: Iterable[str]) -> bytes:
    ...
    if len(parsed) < 2:
        raise ValueError("São necessárias ao menos duas partes")
    out = []
    for pos in range(len(parsed[0][1])):
        ...
        if total > 255:
            raise VaultSecurityError("Partes inválidas")
        out.append(total)
    return bytes(out)
...
class CallbackKeyProvider:
    ...
    def __init__(
        self, wrap: Callable[[bytes, dict[str, str]], bytes], unwrap: Callable[[bytes, dict[str, str]], bytes]
    ):
        self._wrap, self._unwrap = wrap, unwrap

    def wrap(self, key: bytes, context: dict[str, str]) -> bytes:
        return self._wrap(key, context)

    def unwrap(self, key: bytes, context: dict[str, str]) -> bytes:
        return self._unwrap(key, context)
...
class AdvancedVault:
    ROLES = {
        "reader": {"read"},
        "writer": {"read", "write"},
        "rotator": {"read", "write", "rotate"},
        "admin": {"read", "write", "rotate", "delete", "audit"},
    }

    def __init__(self, password: str):
        self.salt = secrets.token_bytes(16)
        self.key = scrypt_kdf(password, self.salt)
        self.entries: dict[str, list[SecretVersion]] = {}
    ...
    def grant(self, actor: str, role: str) -> None:
        ...
        self.bindings.setdefault(actor, set()).add(role)
        self._audit(actor, "grant", role)

    def _allow(self, actor: str, action: str) -> None:
        if not any(action in self.ROLES[r] for r in self.bindings.get(actor, set())):
            raise PermissionError(f"{actor} não pode {action}")
    ...
    def verify_audit(self) -> bool:
        ...
        for item in self.audit:
            ...
            if event["previous"] != prev or not hmac.compare_digest(
                item["signature"],
                hmac.new(self.key, json.dumps(event, sort_keys=True).encode(), hashlib.sha256).hexdigest(),
            ):
                return False
            prev = item["signature"]
        return True
    ...
    def put(
        self, name: str, value: str, actor: str, ttl: int | None = None, metadata: dict[str, Any] | None = None
    ) -> int:
        self._allow(actor, "write")
        versions = self.entries.setdefault(name, [])
        number = len(versions) + 1
        nonce = secrets.token_bytes(12)
        aad = f"{name}:{number}".encode()
        ...
        self._audit(actor, "write", f"{name}@{number}")
        return number

    def get(self, name: str, actor: str, version: int | None = None) -> str:
        self._allow(actor, "read")
        ...
        if item.expires_at and item.expires_at <= time.time():
            raise VaultSecurityError("Segredo expirado")
        plain = chacha20poly1305_decrypt(
            self.key,
            bytes.fromhex(item.nonce),
            bytes.fromhex(item.cipher),
            bytes.fromhex(item.tag),
            f"{name}:{item.version}".encode(),
        )
        self._audit(actor, "read", f"{name}@{item.version}")
        return plain.decode()

    def rotate(self, name: str, generator: Callable[[], str], actor: str, ttl: int | None = None) -> int:
        self._allow(actor, "rotate")
        version = self.put(name, generator(), actor, ttl)
        self._audit(actor, "rotate", name)
        return version

    def leaked(self, candidates: Iterable[str], actor: str) -> list[str]:
        self._allow(actor, "read")
        values = {hashlib.sha256(self.get(n, actor).encode()).digest(): n for n in self.entries}
    ...
    def export_encrypted(self) -> bytes:
        body = json.dumps(
            {
                "salt": self.salt.hex(),
                "entries": {k: [asdict(v) for v in vals] for k, vals in self.entries.items()},
                "bindings": {k: sorted(v) for k, v in self.bindings.items()},
                "audit": self.audit,
            },
            sort_keys=True,
        ).encode()
        nonce = secrets.token_bytes(12)
        cipher, tag = chacha20poly1305_encrypt(self.key, nonce, body, b"vulnscan-sync-v1")
    ...
    def restore_with_salt(cls, blob: bytes, password: str, salt: bytes) -> AdvancedVault:
        obj = cls.__new__(cls)
        obj.salt = salt
        obj.key = scrypt_kdf(password, salt)
        raw = chacha20poly1305_decrypt(obj.key, blob[5:17], blob[33:], blob[17:33], b"vulnscan-sync-v1")
        doc = json.loads(raw)
        obj.entries = {k: [SecretVersion(**v) for v in vals] for k, vals in doc["entries"].items()}
        obj.bindings = {k: set(v) for k, v in doc["bindings"].items()}
        obj.audit = doc["audit"]
        return obj
...
class MemorySecretAgent:
    ...
    def get(self, name: str, actor: str) -> str:
        ...
        value = self.vault.get(name, actor)
        self.cache[key] = (now + self.idle_ttl, value)
        return value
