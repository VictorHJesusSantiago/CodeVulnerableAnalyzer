...
def test_helm_and_kustomize():
    ...
    base = {
        ...
        "kind": "Pod",
        "metadata": {"name": "web"},
        "spec": {"containers": [{"name": "app", "image": "old"}, {"name": "sidecar", "image": "s"}]},
    }
...
def test_api_protocols_and_bfla():
    assert any(
        x["rule_id"] == "ASYNCAPI-002" for x in scan_asyncapi({"asyncapi": "3.0", "servers": {"x": {"url": "ws://x"}}})
    )
...
def test_cfg_ssa_dataflow_intervals_and_symbolic():
    ...
    ssa = to_ssa(cfg)
    targets = [i.target for b in ssa.blocks.values() for i in b.instructions if i.target]
...
def test_memory_lifetime_findings():
    ins = [
        Instruction("alloc", "p", line=1),
        Instruction("free", args=("p",), line=2),
        Instruction("read", args=("p",), line=3),
        Instruction("free", args=("p",), line=4),
    ]
