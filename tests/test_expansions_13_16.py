...
def test_patch_generation_application_and_race(tmp_path):
    ...
    try:
        default_engine().apply(patch, tmp_path)
    except RuntimeError:
        pass
    else:
        assert False
...
def test_lsp_quick_fix_and_llm_provider():
    ...
    class Provider:
        def complete(self, prompt):
            return json.dumps({"explanation": "risco", "replacement": "safe(v)"})
...
def test_false_positive_training_ranking_and_explanation():
    ...
    examples = [
        ({"severity": "high", "confidence": "high", "reachable": True}, True),
...
def test_parallel_stream_ast_automaton_profile_and_limits(tmp_path):
    ...
    cache = ASTCache(str(tmp_path / "ast.db"))
    cache.parse("x=1")
...
def test_distributed_i18n_telemetry_rule_pack(tmp_path):
    q = MemoryQueue()
    worker = DistributedWorker(q, lambda p: {"path": p})
    coord = DistributedCoordinator(q, q)
    ids = coord.submit(["a"])
    worker.run_once()
    assert q.messages["scan.results"][0]["id"] == ids[0]
    assert translate("finding.count", "pt-BR", count=2) == "2 achados"
    telemetry = Telemetry(False)
    telemetry.record("scan", {"file_count": 1})
