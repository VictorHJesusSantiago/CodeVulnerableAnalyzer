...
def test_chacha_aead_and_tamper():
    ...
    cipher, tag = chacha20poly1305_encrypt(key, nonce, b"segredo", b"ctx")
    assert chacha20poly1305_decrypt(key, nonce, cipher, tag, b"ctx") == b"segredo"
    try:
        chacha20poly1305_decrypt(key, nonce, cipher, bytes([tag[0] ^ 1]) + tag[1:], b"ctx")
...
def test_shamir_and_vault_features():
    ...
    agent = MemorySecretAgent(restored)
    assert agent.get("api", "alice") == "token-2"
    agent.lock()
...
def test_identity_graph_and_detection():
    ...
    for p in [
        Principal("u", "user", "ad"),
        Principal("g", "group", "ad"),
        Principal("d", "domain", "ad", {"tier0": True}),
    ]:
        g.add_principal(p)
    g.add_edge(PrivilegeEdge("u", "g", "AddMember", {"write"}))
    g.add_edge(PrivilegeEdge("g", "d", "DCSync", {"replicate"}))
    ...
    risks = detect_identity_risks(
        {"users": [{"id": "u", "mfa": False}], "service_principals": [{"id": "sp", "secrets": ["x"]}]}
    )
...
def test_integrations_quality_gate_queue_and_ux():
    ...
    seen = []
    q.publish("scan", {"id": 1})
    q.consume("scan", seen.append)
    assert seen
    ux = UXState()
    ux.suppress("X", "a", "aceito")
    assert ux.undo_suppression()
