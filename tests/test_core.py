...
def test_engine_detects_dangerous_calls_without_error(tmp_path):
    ...
    """
    res = _scan(tmp_path, "danger.py", 'eval(request.form["x"])\nos.system("rm " + x)\n')
...
def test_yaml_parser_scalars():
    items = _parse_simple_yaml_rules("- id: A\n  multiline: true\n  ignorecase: false\n  num: 5\n")
...
def test_json_csv_junit_markdown_badge(sample_report, tmp_path):
    from analyzer.reporter import export_badge, export_csv, export_json, export_junit, export_markdown
...
def test_baseline_roundtrip(sample_report, tmp_path):
    from analyzer.baseline import compare_with_baseline, save_baseline
